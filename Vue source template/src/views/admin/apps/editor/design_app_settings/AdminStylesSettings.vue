...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      adminProjectsService.getStylesList(this.$route.params.uid).then((res) => {
    ...
    removeItem(index) {
      ...
      adminProjectsService.removeDivForHide(item.id).then(() => {
