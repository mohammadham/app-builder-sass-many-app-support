...
export default {
  ...
  methods: {
    getSigningList() {
      adminProjectsService.getSignList(this.$route.params.uid).then((res) => {
        const data = res.data;
        this.list = data.list;
        this.loading = false;
      }).catch(e => {
    ...
    removeAndroidSign(index) {
      ...
      adminProjectsService.removeAndroidSign(uid).then(() => {
    ...
    removeIosSign(index) {
      this.loadingIndex = index;
      const uid = this.list[index].uid;
      adminProjectsService.removeIosSign(uid).then(() => {
        this.list.splice(index, 1);
        this.loadingIndex = undefined;
      }).catch(e => {
        this.loadingIndex = undefined;
