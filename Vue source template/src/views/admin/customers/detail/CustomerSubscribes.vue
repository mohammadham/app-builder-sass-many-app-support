      ...
      >
        <div v-for="(subscribe) in list">
          <v-list-item
            lines="two"
            density="default"
            :to="`/admin/transactions/subscribe/${subscribe.subscribe_external_id}`"
          >
...
export default {
  ...
  methods: {
    getSubscribesList() {
      ...
      adminSubscribeService.getUserSubscribes(this.offset, this.$route.params.customer_id).then((res) => {
        const data = res.data;
