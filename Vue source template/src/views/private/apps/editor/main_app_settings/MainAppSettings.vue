      ...
      <v-menu location="center">
        ...
        <v-list>
          <v-list-item @click="modalRemoveIsOpen = true">
            <v-list-item-title class="text-red-accent-3">{{ $tr('project', 'key_388') }}</v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </template>
...
export default {
  ...
  methods: {
    getSettingsData() {
      projectsService.getMainInfo(this.$route.params.uid).then((res) => {
        const data = res.data;
        this.name = data.name;
        this.link = data.link;
    ...
    updateMain() {
      this.isUpdate = true;
      projectsService.updateMainSettings(this.$route.params.uid, {
        link: this.link,
        name: this.name,
        app_id: this.appId,
